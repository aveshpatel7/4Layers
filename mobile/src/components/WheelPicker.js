...
const CONTAINER_HEIGHT = 132; // 3 items visible simultaneously (Center item highlighted)

export const WheelColumn = ({ data, selectedValue, onValueChange, flex = 1 }) => {
  ...
  const handleMomentumScrollEnd = useCallback((event) => {
    ...
    if (data[index]) {
      onValueChange(data[index].value !== undefined ? data[index].value : data[index]);
    }
  }, [data, onValueChange]);
